...
def _get_train_and_test_score_per_estimator_on(
        X, y, working_dir, cv, scoring, time_limit, framework, device, n_jobs, task, kwargs_per_classifier
) -> dict[str, tuple[float, float]]:
    # lower and strip the method names
    framework = framework.lower().strip() 

    results = {}
    estimator = get_estimators_with(working_dir, time_limit, device, framework, task, kwargs_per_classifier)[framework]
